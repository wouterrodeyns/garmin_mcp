...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"workoutTargetTypeId": 6, "workoutTargetTypeKey": "power.between"},
        {"workoutTargetTypeId": 9, "workoutTargetTypeKey": "power.between"},
    ],
)
...
async def test_upload_cycling_workout_pace_target_remains_valid(
    ...
    """The power fix must not block an intentional speed target."""
    import json as json_module
...
async def test_upload_cycling_workout_rejects_zone_and_range_during_normalization(
    ...
    """Shared normalization rejects a named power zone plus custom bounds."""
    workout_data = _cycling_workout_with_steps(
        [{
            "type": "ExecutableStepDTO",
            "stepOrder": 1,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"targetValueTwo": 250}, "requires both targetValueOne and targetValueTwo"),
        (
        ...
        ({"targetValueOne": 200, "targetValueTwo": -1}, "must be non-negative"),
        ({"targetValueOne": float("inf"), "targetValueTwo": 250}, "must be finite"),
        ({"targetValueOne": float("nan"), "targetValueTwo": 250}, "must be finite"),
        ...
        ({"zoneNumber": "4"}, "must be an integer between 1 and 7"),
        ({"zoneNumber": 0}, "must be an integer between 1 and 7"),
...
@pytest.mark.asyncio
async def test_upload_cycling_workout_rejects_invalid_power_range_in_repeat_group(
    app_with_workouts, mock_garmin_client
):
    """Nested executable steps receive the same power validation."""
    nested_step = {
        "type": "ExecutableStepDTO",
        "stepOrder": 1,
        "stepType": {"stepTypeId": 3, "stepTypeKey": "interval"},
    ...
    result = await app_with_workouts.call_tool(
        "upload_workout",
        {"workout_data": _cycling_workout_with_steps([repeat])},
    )

    assert "workoutSegments[0].workoutSteps[0].workoutSteps[0].targetType" in result[0][0].text
    assert "power target low value cannot exceed high value" in result[0][0].text
    mock_garmin_client.upload_workout.assert_not_called()
